# Salut, moi c'est Nekokimi0 👋

Étudiante en L3 Informatique à l'Université Lyon 2, je recherche une alternance de 2 ans dans le développement web.  
J'aime autant coder que créer — du backend PHP à la modélisation 3D sur Blender !

---

## 🛠️ Mes technos

**Langages**  
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat&logo=css3&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**Outils & logiciels**  
![Blender](https://img.shields.io/badge/Blender-F5792A?style=flat&logo=blender&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![Unity](https://img.shields.io/badge/Unity-000000?style=flat&logo=unity&logoColor=white)
![Adobe Illustrator](https://img.shields.io/badge/Illustrator-FF9A00?style=flat&logo=adobeillustrator&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

**Concepts**  
`MVC` `POO` `Architecture MVC` `CRUD` `Sessions`

---

## 📁 Mes projets

| Projet | Description | Technos |
|:-|:-|:-|
| [Le Ronron Café](https://github.com/Aid4n4/Projet-php-bar-a-chat) | Site web d'un bar à chats fictif — réservations, gestion admin, upload de photos | PHP, MariaDB, MVC |
| [Character Finder](https://github.com/Aid4n4/Projet-info-graphique-et-vision) | Détection de personnages par vision par ordinateur | Python, OpenCV, SIFT |
| [Diorama Blender](https://github.com/Aid4n4/Diorama-Blender) | Diorama 3D d'une chambre de chats | Blender, Cycles |
| [Fiole Magique](https://github.com/Aid4n4/Fiole-Blender) | Modélisation d'une fiole fantasy | Blender, Cycles |
| [My Virtual Zoo](https://github.com/Aid4n4/My-virtual-zoo) | Jeu de gestion de zoo en mode textuel | C++ |

---

## 🎯 Ce que je recherche

Je suis à la recherche d'une **alternance de 2 ans** en développement web à partir de 2025.  
Mes points forts : backend PHP/MVC, bases de données relationnelles, et un vrai souci du code bien structuré.  
Ouverte à tout projet qui mêle technique et créativité ! 🐾

---

## 📬 Me contacter

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/séréna-pot-9a6499334)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:serena.pot@gmail.com)
